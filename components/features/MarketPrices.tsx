...
export const MarketPrices: React.FC<MarketPricesProps> = ({ userLocation, onLocationChange, theme, onDetectLocation, lang }) => {
    const [crops, setCrops] = useState('Wheat,Rice,Corn');
    ...
    useEffect(() => {
        const cached = getCachedData<MarketPrice[]>('marketPrices');
        if (cached) {
    ...
    const fetchPrices = useCallback(async () => {
        ...
        try {
            ...
            const result = await getMarketPrices(location, cropList, lang);
            setPriceData(result);
            setCachedData('marketPrices', result);
            setLastUpdated(new Date().toISOString());
        } catch (err) {
            setError(t('marketPrices.fetchError'));
            console.error(err);
        } finally {
            setLoading(false);
        }
    }, [location, crops, lang, t, isOnline]);
    ...
    const tooltipText = theme === 'dark' ? '#f3f4f6' : '#374151';
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-8 dark:bg-gray-800">
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    ...
                    <div>
                        <label htmlFor="crops" className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('marketPrices.crops')}</label>
                        <input type="text" id="crops" value={crops} onChange={(e) => setCrops(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-light-green focus:border-brand-light-green text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
                    </div>
                    <button type="submit" disabled={loading || !location || !isOnline} className="w-full bg-brand-green hover:bg-brand-light-green text-white font-bold py-2 px-4 rounded-md transition-colors disabled:bg-gray-400">
                        {loading ? t('marketPrices.fetching') : t('marketPrices.updatePrices')}
                    </button>
                </form>
            </div>
            ...
            {!loading && !error && (
                <div className="bg-white p-6 rounded-lg shadow-lg dark:bg-gray-800">
                    <h3 className="text-xl font-bold text-brand-green mb-4 dark:text-brand-light-green">{t('marketPrices.chartTitle')}</h3>
                    ...
                    {priceData.length > 0 ? (
                        <div style={{ width: '100%', height: 400 }}>
                           <ResponsiveContainer>
                                <BarChart data={priceData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" stroke={gridColor} />
                                    <XAxis dataKey="crop" tick={{ fill: tickColor }} />
                                    ...
                                    <Bar dataKey="price" fill="#38a169" name={t('marketPrices.price')} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    ) : (
                         <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                            <h2 className="text-2xl font-semibold mb-2">{t('marketPrices.noDataTitle')}</h2>
                            <p>{t('marketPrices.noDataDescription')}</p>
                        </div>
                    )}
                </div>
            )}
